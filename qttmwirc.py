  ...
  def keyPressEvent(self, event):
    if event.key() == Qt.Key_Return and not (event.modifiers() and Qt.ShiftModifier):
      ...
      if text.lstrip().startswith("/"):
        ...
        if self.channel.network.factoryinstance.serverconnection:
...
def docommand(windowtype, window, text): #todo: test from server, channel, and privmsg
  ...
  if params[0].lower() == "/join":
    if len(params) in (2, 3):
      if window.network.factoryinstance.serverconnection:
  ...
  def keyPressEvent(self, event):
    if event.key() == Qt.Key_Return and not (event.modifiers() and Qt.ShiftModifier):
      ...
      if text.lstrip().startswith("/"):
        ...
        if self.messagewindow.network.factoryinstance.serverconnection:
...
class ServerConnection(irc.IRCClient):
  ...
  def irc_RPL_WELCOME(self, prefix, params):
    if params[1].startswith("Welcome to the "):
      ...
      oldnetworkname = getattr(self, networkname, None) #should networkname go under network, factoryinstance or serverconnection? 
      ...
      if oldnetworkname and (networkname.lower() != oldnetworkname.lower()): #calling .lower is the Right Thing to do, right? 
        ...
        #todo: we may also want to do something if networkname.lower() != self.factoryinstance.network.config["name"].lower()
...
class ServerFactory(protocol.ReconnectingClientFactory):
  def __init__(self, nickname="qttmwirc", password=None, username="qttmwirc", realname=None):
...
for network_config in config["networks"]:
  ...
  factoryinstance = ServerFactory(nickname=nick, username=username, password=password, realname=realname)
